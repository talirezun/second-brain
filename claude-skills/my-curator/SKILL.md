---
name: my-curator
description: Use whenever the user wants to interact with their My Curator second brain — a markdown knowledge graph managed by The Curator app via the my-curator MCP. Activates for READ ("what does my wiki/cohort wiki say about X", "deep research my second brain", "find every source that mentions Y"), WRITE ("add to my second brain", "save to my wiki", "compile our findings", "put this in my projects domain"), Shared Brain contribution ("save this to our shared brain", "contribute to the cohort wiki"), and maintenance ("check my wiki", "find broken links", "scan for duplicate pages"). Enforces atomic decomposition (entities, concepts, summaries), grounds every wikilink in an existing slug before writing, refuses speculative links on fresh domains, respects per-domain siloing (no cross-domain links), and handles Shared Brain mirrors correctly (read-only locally; contributions go through the user's personal opted-in domain + Sync-tab Push). Always calls list_domains and get_index BEFORE composing any write.
